# Testing for lake profiles app

library(dplyr)

load("D:/R/LakeProfileDashboard-master/modified_app/data/assessed_profs.rdata")

profiles_long <- assessed_profs[[4]]

profiles_wide <- assessed_profs[[3]]

prof_assess_ind <- assessed_profs[[2]]

## Data work before map within server(): ------------------------------------------
...
# above but dplyr syntax:
lake_aus2 <- au_poly %>% 
  filter(AU_Type == "Reservoir/Lake")

# Extract site locations
prof_sites = unique(assessed_profs$profile_asmnts_mlid_param[, c(
  "ASSESS_ID",
  "AU_NAME",
  "R317Descrp",
  "BEN_CLASS",
...
profiles_long = unique(profiles_long[, c(
  "DataLoggerLine",
  "ActivityIdentifier",
  "ActivityStartDate",
  "R3172ParameterName",
...
profiles_long$ActivityStartDate = as.Date(profiles_long$ActivityStartDate, format =
                                            '%Y-%m-%d')
...
profiles_wide$ActivityStartDate = as.Date(profiles_wide$ActivityStartDate, format =
                                            '%Y-%m-%d')

# Calc max depth for each profile
max_depth = aggregate(
  Depth_m ~ ActivityIdentifier,
  data = profiles_wide,
  FUN = 'max',
  na.rm = T
)
...
ind_prof_asmnts$ActivityStartDate = as.Date(ind_prof_asmnts$ActivityStartDate, format =
                                              '%Y-%m-%d')
...
mlid_param_asmnts = mlid_param_asmnts[, !names(mlid_param_asmnts) %in% c("IR_Lat",
                                                                         "IR_Long",
                                                                         "BEN_CLASS",
                                                                         "R317Descrp",
                                                                         "IR_MLNAME",
...
observe({
  ...
  reactive_objects$sel_profiles = profiles_long[profiles_long$MonitoringLocationIdentifier ==
...
observe({
  ...
  table_data = profiles_wide[profiles_wide$ActivityIdentifier == reactive_objects$selectedActID, c(
    "IR_MLID",
    "ActivityStartDate",
    "Depth_m",
    "DO_mgL",
...
observe({
  prof_table_proxy %>% DT::hideCols(hide = which(
    names(reactive_objects$table_data) %in% c("do_exc", "pH_exc", "temp_exc")
  ))
})
...
observe({
  ...
  selected_prof_asmnts = ind_prof_asmnts[ind_prof_asmnts$IR_MLID == reactive_objects$sel_mlid &
  ...
  reactive_objects$sel_profs_wide = profiles_wide[profiles_wide$IR_MLID == reactive_objects$sel_mlid &
...
output$hab_width = renderPlot({
  ...
  if (dim(reactive_objects$selected_prof_asmnts)[1] > 0) {
    ...
    plot(
      max_hab_width ~ ActivityStartDate,
      data = reactive_objects$selected_prof_asmnts,
      pch = NA,
      cex = 1.5,
      ylab = "Width (m)",
      xlab = "",
      ...
      ylim = c(
        0,
        max(
          reactive_objects$selected_prof_asmnts$max_depth_m,
          na.rm = T
        )
      )
    )
    ...
    if (input$show_dates) {
      axis(
        1,
        at = unique(
          reactive_objects$selected_prof_asmnts$ActivityStartDate
        ),
        labels = unique(
          as.Date(
            reactive_objects$selected_prof_asmnts$ActivityStartDate
    ...
    legend(
      ...
      lwd = c(2, 1),
      cex = 1.5,
      legend = c("Max depth", "DO/temp habitat")
    )
    par(xpd = FALSE)
  }
})
...
output$pct_exc = renderPlot({
  ...
  if (dim(reactive_objects$selected_prof_asmnts)[1] > 0) {
    ymax = max(5, max(
      max(
        reactive_objects$selected_prof_asmnts$do_pct_exc,
        na.rm = T
      ),
      max(
        reactive_objects$selected_prof_asmnts$temp_pct_exc,
        na.rm = T
      ),
      max(
        reactive_objects$selected_prof_asmnts$ph_pct_exc,
        na.rm = T
      )
    ) * 1.1)
    ...
    points(
      temp_pct_exc ~ ActivityStartDate,
      data = reactive_objects$selected_prof_asmnts,
      pch = 21,
      bg = "orange",
      type = 'b',
    ...
    legend(
      ...
      pch = c(24, 21, 22),
      pt.bg = c("deepskyblue3", "orange", "green"),
      cex = 1.5
    )
    par(xpd = FALSE)
  }
...
output$heatmap = renderPlot({
  req(reactive_objects$sel_profs_wide,
      reactive_objects$sel_profiles)
  if (dim(reactive_objects$sel_profs_wide)[1] > 0) {
    if (length(unique(reactive_objects$sel_profs_wide$ActivityStartDate)) ==
        1) {
      plot.new()
      text(0.5,
           0.5,
           "Only one profile date available. Cannot interpolate.")
      box()
    } else{
      ...
      # Define criteria
      if (input$heatmap_param != "DO-temperature habitat profile width") {
        criteria = unique(reactive_objects$sel_profiles[reactive_objects$sel_profiles$R3172ParameterName ==
      ...
      profileHeatMap(
        reactive_objects$sel_profs_wide,
        parameter = parameter,
